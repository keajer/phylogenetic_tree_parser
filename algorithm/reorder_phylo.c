			  ...
			  int *edge2, int *nedge, int *neworder)
{
    ...
    printf("in neworder_pruningwise\n");
    ...
    printf("calculating ready\n");
    for (i = 0; i < *nedge; i++)
      if (edge2[i] <= *ntip) ready[i] = 1;
      else ready[i] = 0;
    printf("done calculating ready\n");
    ...
    while (nextI < *nedge - Ndegr[*ntip]) {
		...
		n = 1;
	        ...
	        printf("counting %d\n",i);
	        if (edge1[i] == node){
                       printf("edge1[i] == node i=%d edge1[i]=%d node=%d\n",i,edge1[i],node);
                       n++;
                    }
		else {
