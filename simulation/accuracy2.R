...
#increase alpha to be 0.15, so the prob. for exceed 3/4 is
pij <- 1 - (1/4 + 3/4*exp(-4*4.4*0.15))
(prob <- 1- pbinom(750, 1000, pij))#7.83246e-05
...
#split 0.15 into 10 levels of alpha, we will have
# alpha = 0.015, 0.03, 0.045, 0.06, 0.075, 0.09, 
...
x <- c(0.015, 0.03, 0.045, 0.06, 0.075, 0.09, 0.105, 0.12, 0.135, 0.15)
plot(x, ynj, type = 'o', xlab = "alpha", ylab = "accuracy rate", ylim = c(0,1))
lines(x, ynew, type = 'o', col = 2)
title(main = "Neighbor-Joining vs. New Algorithm")
legend(locator(1), c('Neighbor-joining', 'New Algorithm'), lty = 1, col = c(1,2))